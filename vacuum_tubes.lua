		...
		and lua_entity.name == "__builtin:item" then
			...
			and obj_pos.z >= minpos.z and obj_pos.z <= maxpos.z then
				if lua_entity.itemstring ~= "" then
					pipeworks.tube_inject_item(pos, pos, vector.new(0, 0, 0),
		...
		node_def = {
			...
			on_construct = function(pos)
				...
				meta:set_string("formspec", "size[2.1,0.8]"..
						...
						default.gui_bg..
						default.gui_bg_img)
				meta:set_string("infotext", "Adjustable Vacuuming " ..
					"Pneumatic Tube Segment")
			end,
			on_receive_fields = function(pos, fields, sender)
				...
				meta:set_string("infotext", ("Adjustable Vacuuming " ..
					"Pneumatic Tube Segment (%dm)"):format(dist))
			end,
			on_destruct = function(pos)
				if get_vacuuming(pos) then
					remove_vacuuming(pos)
					...
					for x = math.floor(pos.x-maxradius), math.ceil(pos.x+maxradius) do
						...
						if r then
							...
							and vector.length(objtotube) <= r then
								pipeworks.tube_inject_item(tpos, tpos,
									vector.new(0, 0, 0), lua_entity.itemstring)
								lua_entity.itemstring = ""
								obj:remove()
								return true
							end
